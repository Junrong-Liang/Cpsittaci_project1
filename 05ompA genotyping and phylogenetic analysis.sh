...
# ==============================================
# STEP 9: OMPA GENOTYPING AND PHYLOGENETIC ANALYSIS
# ==============================================
echo "Step 9: ompA genotyping for ${SAMPLE_ID}"

# Align reads to ompA reference
bwa mem \
    -t ${NUM_THREADS} \
    ${REFERENCE_DIR}/ompA_ref.fasta \
    ${OUTPUT_DIR}/${SAMPLE_ID}/alignment/${SAMPLE_ID}_cp_candidate.fastq.gz \
    2>&1 | samtools view -bS - \
    | samtools sort -o ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_aligned.bam
...
samtools mpileup \
    ...
    -B \
    -Q 0 \
    -f ${REFERENCE_DIR}/ompA_ref.fasta \
    ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_aligned.bam \
    | ivar consensus \
    -p ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_consensus \
    -n N \
    -m 1 \
    -t 0.5

# Multiple sequence alignment
cat ${REFERENCE_DIR}/ompA_reference_sequences.fasta \
    ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_consensus.fa \
    > ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_all.fasta

mafft \
    --auto \
    --thread ${NUM_THREADS} \
    ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_all.fasta \
    > ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_aligned.fasta
...
fasttree \
    ...
    -gtr \
    ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_aligned.fasta \
    > ${OUTPUT_DIR}/${SAMPLE_ID}/phylogeny/${SAMPLE_ID}_ompA_tree.newick
